...
describe("gpt-image stdio MCP server", () => {
  ...
  it("exposes the model-line tools over the real stdio transport", async () => {
    ...
    client = new Client({ name: "gpt-image-mcp-test", version: "0.1.0" });
    ...
    for (const model of ["gpt-image-1.5"]) {
    ...
    const multiEndpointModels: Record<string, string[]> = {"gpt-image-1.5":["edit_image","text_to_image"]};
